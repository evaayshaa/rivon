...
export const getBasketThunk=createAsyncThunk('get/basket',async()=>{
    const res=await axios.get('http://localhost:5050/basket')
    return res.data
})

export const postBasketThunk=createAsyncThunk('post/basket',async(data)=>{
    const res=await axios.post('http://localhost:5050/basket',data)
    return res.data
})
export const deleteBasketThunk=createAsyncThunk('delete/basket',async(id)=>{
    const res=await axios.delete(`http://localhost:5050/basket/${id}`)
    return id
})
